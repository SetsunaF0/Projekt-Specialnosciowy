...
if (isset($_POST['uname']) && isset($_POST['password'])
	...
	if (empty($uname)) {
        ...
        header("Location: signup.php?error=Powtórzone hasło jest niepoprawne&$user_data");
	    exit();
	}
		...
		if (mysqli_num_rows($result) > 0) {
			header("Location: signup.php?error=Nazwa użytkownmika jest zajęta&$user_data");
	        exit();
		}else {
           $sql2 = "INSERT INTO users(user_name, password, name) VALUES('$uname', '$pass', '$name')";
           $result2 = mysqli_query($conn, $sql2);
           if ($result2) {
           	 header("Location: signup.php?success=Twoje konto zostało utworzone");
	         exit();
           }else {
